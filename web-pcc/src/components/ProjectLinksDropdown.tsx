      ...
      <div className="w-full max-w-lg rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-700">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Zapisz nazwê i adres, który ma siê otwieraæ po klikniêciu.</p>
          </div>
        ...
        <form className="space-y-5 p-6" onSubmit={handleSave}>
          ...
          <div>
            <label className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-gray-400">Widoczne dodatkowo w zak³adkach</label>
          ...
          <div className="flex items-center justify-between border-t border-gray-100 pt-4 dark:border-gray-700">
            <div>
              {linkToEdit && <button type="button" onClick={handleDelete} className="inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"><Trash2 size={16} />Usuñ</button>}
            </div>
        ...
        {isOpen && (
          <div className="absolute right-0 top-full z-30 mt-2 w-[22rem] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="max-h-80 overflow-y-auto p-3">
              {isLoading ? <div className="flex items-center justify-center py-8 text-gray-400"><Loader2 size={18} className="animate-spin" /></div> : filtered.length === 0 ? <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 px-4 py-6 text-center text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-900/30 dark:text-gray-400">Brak zapisanych linków dla tej zak³adki.</div> : <div className="space-y-2">{filtered.map((link) => <div key={link.id} className="flex items-center gap-2 rounded-xl border border-gray-100 bg-gray-50/80 px-3 py-2.5 dark:border-gray-700 dark:bg-gray-900/30"><a href={link.url} target="_blank" rel="noreferrer" className="group min-w-0 flex-1 text-left"><div className="flex items-center gap-2 text-sm font-semibold text-gray-900 group-hover:text-indigo-600 dark:text-white dark:group-hover:text-indigo-300"><span className="truncate">{link.name}</span><ExternalLink size={13} className="shrink-0 opacity-60" /></div></a><button type="button" onClick={() => { setEditingLink(link); setIsModalOpen(true); setIsOpen(false); }} className="rounded-lg p-2 text-gray-400 hover:bg-white hover:text-indigo-600 dark:hover:bg-gray-800 dark:hover:text-indigo-300"><Edit2 size={15} /></button></div>)}</div>}
            </div>
          </div>
        )}
      </div>
